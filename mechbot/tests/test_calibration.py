...
class GradientDescentTests(unittest.TestCase):
    def test_raw_descent(self):
        ...
        initial_m2 = StepperMotor((-2.2, 1.94), 200, 1, -0.65)
        initial_device = VirtualDevice(initial_m1, initial_m2, .14, 0.)
...
def device_dist(first, second):
    ...
    m1 = error(vec_len(first.motor1.pos), vec_len(second.motor1.pos))
    m2 = error(vec_len(first.motor2.pos), vec_len(second.motor2.pos))
    a1 = error(first.motor1.align, second.motor1.align)
    a2 = error(first.motor2.align, second.motor2.align)
    gp = error(first.gap - first.stick, second.gap - second.stick)

    return vec_len([m1, m2, a1, a2, gp])
